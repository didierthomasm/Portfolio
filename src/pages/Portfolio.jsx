...
const images = [
  ...
  {src: portfolioImages.pto4, alt: 'Password Generator', href: 'https://didierthomasm.github.io/js-awesome-password-generator'},
    ...
    <main className='grid md:mt-16 md:grid-cols-3 gap-4 mt-32 grid-cols-1'>
      {images.map((image) => (
        <div key={image.src} className='flex justify-center shadow-2xl w-38 hover:drop-shadow-2xl'>
          <a href={image.href} target='_blank' rel='noopener noreferrer'><img src={image.src} alt={image.alt} className='h-36'/></a>
        </div>
      ))}
    </main>
  )
